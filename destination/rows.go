...
func (r *connRow) Scan(dest ...any) (err error) {
	...
	for _, d := range dest {
		if _, ok := d.(*DriverBytes); ok {
			rows.Close()
			return fmt.Errorf("cannot scan into *DriverBytes from QueryRow")
		}
	}
...
func (rows *baseRows) FieldDescriptions() []FieldDescription {
...
func (rows *baseRows) CommandTag() CommandTag {
...
func (rows *baseRows) Scan(dest ...any) error {
	...
	if rows.scanPlans == nil {
		rows.scanPlans = make([]ScanPlan, len(values))
...
func ScanRow(typeMap *Map, fieldDescriptions []FieldDescription, values [][]byte, dest ...any) error {
	if len(fieldDescriptions) != len(values) {
...
func RowsFromResultReader(typeMap *Map, resultReader *ResultReader) Rows {
	return &baseRows{
		typeMap:      typeMap,
		resultReader: resultReader,
	}
...
func ForEachRow(rows Rows, scans []any, fn func() error) (CommandTag, error) {
	defer rows.Close()
...
func joinFieldNames(fldDescs []FieldDescription) string {
	switch len(fldDescs) {
	case 0:
		return ""
	case 1:
...
func fieldPosByName(fldDescs []FieldDescription, field string) (i int) {
	i = -1
